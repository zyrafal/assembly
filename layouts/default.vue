  ...
  setup() {
    ...
    watch(
      ...
      async () => {
        ...
        if (route.value.path.includes(['mainnet', 'polygon', "arbitrum", "avalanche", "hardhat"]) && route.value.path.includes(activeNetwork.value.id)) {
          redirect('/')
        }
      }, { immediate: true })
    ...
    watch(chainId, (val) => {
      if (val) {
        if ([1, 137, 42161, 43114, 31337].includes(val)) {
